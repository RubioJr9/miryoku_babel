...
#+NAME: thumbs
| U_NP | U_NP | BSPC | E | ESC | ENT | SPC | TAB | U_NP | U_NP |
...
|      | ALGR      | LOCK_OPP    | U_MEDIA    | U_FUN   | LOCK_CUR | LOCK_OPP   | ALGR        |           |      |
| U_NP | U_NP      | U_BRAC      | U_NAV      | U_BNFRD | U_NSYM   | U_PUNC     |             | U_NP      | U_NP |
...
#+NAME: rsthp
| TG(TRACKBALL) | C | Y | F | K | Z | L | , | U   | Q             |
| R             | S | T | H | P | W | N | A | I   | O             |
| J             | V | G | D | B | X | M | . | DEL | TG(TRACKBALL) |
...
| TG(BASE) |      |      |      |      |   |      |      |      |          |
|          |      |      |      |      |   |      |      |      |          |
|          |      |      |      |      |   | BTN1 | BTN2 | BTN3 | TG(BASE) |
| U_NP     | U_NP | BTN2 | BTN1 | BTN3 |   |      |      | U_NP | U_NP     |
****** Media
...
#+NAME: nonkc
| U_ | RGB_ | OU_  | QK_ | S( | C( | SCMD( | LCMD( | TD( | CW_TOGG | TG( |
...
<<table-layer-half(current_layer_name="U_NSYM", half_table=nsym-l, mode="l")>>

#define MIRYOKU_ALTERNATIVES_BRAC \
<<table-layer-half(current_layer_name="U_BRAC", half_table=brac-r, mode="r")>>

#define MIRYOKU_ALTERNATIVES_PUNC \
<<table-layer-half(current_layer_name="U_PUNC", half_table=punc-l, mode="l")>>
